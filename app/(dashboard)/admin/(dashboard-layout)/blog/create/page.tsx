...
export default function BlogCMS() {
  ...
  const handleSave = async () => {
    try {
      // 1. apiClient handles JSON.stringify and Content-Type automatically
      // 2. Token is automatically attached via the interceptor
      await apiClient.post("/api/blog-details", {
        ...mainData,
        sections,
      });
      ...
      // Axios throws on 4xx/5xx errors, unlike fetch
      console.error("Save failed:", error.response?.data || error.message);
      alert(
        "Failed to publish blog. Please check your connection or login status."
      );
    }
  };
