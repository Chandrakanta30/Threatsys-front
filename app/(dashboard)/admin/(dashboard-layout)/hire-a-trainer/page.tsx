...
export default function HireTrainerAdmin() {
  ...
  const loadData = async () => {
    const comp = await apiClient.get("/api/hire-trainer/comparison");
    const step = await apiClient.get("/api/hire-trainer/steps");
  ...
  const removeComparison = async (row: any, index: number) => {
    if (row.id) {
      await apiClient.delete(`/api/hire-trainer/comparison?id=${row.id}`);
    }
  ...
  const saveComparison = async () => {
    for (const row of comparison) {
      if (row.id) {
        await apiClient.put("/api/hire-trainer/comparison", row);
      } else {
        await apiClient.post("/api/hire-trainer/comparison", row);
      }
    }
  ...
  const removeStep = async (step: any, index: number) => {
    if (step.id) {
      await apiClient.delete(`/api/hire-trainer/steps?id=${step.id}`);
    }
  ...
  const saveSteps = async () => {
    for (const step of steps) {
      if (step.id) {
        await apiClient.put("/api/hire-trainer/steps", step);
      } else {
        await apiClient.post("/api/hire-trainer/steps", step);
      }
    }
